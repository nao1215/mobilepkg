...
func TestRequireFields(t *testing.T) {
	...
	t.Run("no violations when all fields present", func(t *testing.T) {
		...
		vs := mobilepkg.ValidateReport(report, []mobilepkg.Rule{
			mobilepkg.RequireFields("identifier", "display_name", "version_marketing", "version_build", "min_sdk", "target_sdk"),
		})
		if len(vs) != 0 {
	...
	t.Run("reports missing fields", func(t *testing.T) {
		...
		vs := mobilepkg.ValidateReport(report, []mobilepkg.Rule{
			mobilepkg.RequireFields("identifier", "display_name", "version_marketing"),
		})
		if len(vs) != 2 {
	...
	t.Run("unknown fields are ignored", func(t *testing.T) {
		...
		report := mobilepkg.Report{}
		vs := mobilepkg.ValidateReport(report, []mobilepkg.Rule{
			mobilepkg.RequireFields("nonexistent_field"),
		})
		if len(vs) != 0 {
			t.Errorf("expected 0 violations for unknown field, got %d", len(vs))
		}
	})
}
...
func TestRuleFunc(t *testing.T) {
	...
	custom := mobilepkg.RuleFunc(func(r mobilepkg.Report) []mobilepkg.Violation {
	...
	t.Run("custom rule passes", func(t *testing.T) {
		t.Parallel()
		report := mobilepkg.Report{Platform: mobilepkg.PlatformAndroid}
		vs := mobilepkg.ValidateReport(report, []mobilepkg.Rule{custom})
		if len(vs) != 0 {
			t.Errorf("expected 0 violations, got %d", len(vs))
		}
	})
	...
	t.Run("custom rule fails", func(t *testing.T) {
		t.Parallel()
		report := mobilepkg.Report{Platform: mobilepkg.PlatformIOS}
		vs := mobilepkg.ValidateReport(report, []mobilepkg.Rule{custom})
		if len(vs) != 1 {
...
func TestValidateReport_MultipleRules(t *testing.T) {
	...
	rules := []mobilepkg.Rule{
		mobilepkg.RequireFields("display_name"),
		mobilepkg.VersionFormat(`^\d+\.\d+\.\d+$`),
		mobilepkg.PermissionDenyList("android.permission.SEND_SMS"),
	}

	vs := mobilepkg.ValidateReport(report, rules)
	if len(vs) != 3 {
...
func TestValidateReport_NoRules(t *testing.T) {
	...
	report := mobilepkg.Report{}
	vs := mobilepkg.ValidateReport(report, nil)
	if len(vs) != 0 {
