...
func InspectAAB(r io.ReaderAt, size int64, sections uint64, iconSizePx int) (*Result, []Diagnostic, error) {
	result := &Result{}
	...
	xmlNode, walkErr := parseAABManifestXML(r, size)
	if walkErr != nil {
...
func parseAABManifestXML(r io.ReaderAt, size int64) (*pb.XmlNode, error) {
	zr, err := zip.NewReader(r, size)
	if err != nil {
		return nil, err
	}

	data, err := readZipFile(zr, "base/manifest/AndroidManifest.xml")
	if err != nil {
		return nil, err
	}
