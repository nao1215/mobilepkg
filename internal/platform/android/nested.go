...
// into memory.
func openNestedZip(zr *zip.Reader, name string) (*zip.Reader, error) {
...
func findNestedAPK(zr *zip.Reader, candidates []string) (*zip.Reader, error) {
	for _, name := range candidates {
		inner, err := openNestedZip(zr, name)
		if err == nil {
			return inner, nil
		}
	}
