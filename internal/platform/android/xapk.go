...
func InspectXAPK(zr *zip.Reader, sections uint64) (*Result, []Diagnostic, error) {
	...
	baseZR, baseErr := findXAPKBaseAPK(zr, xm)
	if baseErr == nil {
		innerResult, innerDiags, innerErr := Inspect(baseZR, sections, nil, 0)
		if innerErr == nil {
...
func findXAPKBaseAPK(zr *zip.Reader, xm xapkManifest) (*zip.Reader, error) {
	...
	return findNestedAPK(zr, candidates)
}
