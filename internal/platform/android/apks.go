...
func InspectAPKS(zr *zip.Reader, sections uint64) (*Result, []Diagnostic, error) {
	baseZR, err := findBaseMasterAPK(zr)
	if err != nil {
		return nil, nil, fmt.Errorf("android/apks: %w", err)
	}
	return Inspect(baseZR, sections, nil, 0)
}

// findBaseMasterAPK locates the base APK within an APKS archive.
// It tries well-known paths in order of likelihood.
func findBaseMasterAPK(zr *zip.Reader) (*zip.Reader, error) {
	// Primary: exact name used by bundletool
	if inner, err := openNestedZip(zr, "splits/base-master.apk"); err == nil {
		return inner, nil
	}
	...
	for _, f := range zr.File {
		if strings.HasPrefix(f.Name, "splits/base") && strings.HasSuffix(f.Name, ".apk") {
			if inner, err := openNestedZip(zr, f.Name); err == nil {
				return inner, nil
			}
		}
	}
	...
	if inner, err := openNestedZip(zr, "universal.apk"); err == nil {
		return inner, nil
	}
