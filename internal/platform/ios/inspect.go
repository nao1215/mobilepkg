...
func Inspect(zr *zip.Reader, sections uint64, maxEntryBytes int64) (*Result, []Diagnostic, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("%w: %w", ErrInfoPlistNotFound, err)
	}

	plistData, err := readZipFile(zr, infoPlistPath, maxEntryBytes)
	if err != nil {
		return nil, nil, fmt.Errorf("%w: %w", ErrInfoPlistNotFound, err)
	}
	...
	if _, err := plist.Unmarshal(plistData, &info); err != nil {
		return nil, nil, fmt.Errorf("%w: %w", ErrInfoPlistParseFailed, err)
	}
	...
	if sections&((1<<bitPermissions)|(1<<bitPlatformRaw)) != 0 {
		entData, entErr := readZipFile(zr, appDir+"embedded.mobileprovision", maxEntryBytes)
		if entErr == nil {
	...
	if sections&(1<<bitIcon) != 0 {
		iconPath, iconData, iconErr := extractIcon(zr, info, appDir, maxEntryBytes)
		if iconErr != nil {
	...
	if sections&(1<<bitSigning) != 0 {
		provData, provErr := readZipFile(zr, appDir+"embedded.mobileprovision", maxEntryBytes)
		if provErr == nil {
...
func extractIcon(zr *zip.Reader, info map[string]any, appDir string, maxEntryBytes int64) (string, []byte, error) {
	...
	for _, name := range iconNames {
		...
		for _, c := range candidates {
			p := appDir + c
			data, err := readZipFile(zr, p, maxEntryBytes)
			if err == nil {
				return p, data, nil
			}
		}
	...
	for _, f := range zr.File {
		if strings.HasPrefix(f.Name, appDir) && strings.Contains(f.Name, "Icon") &&
			...
			data, err := readZipFile(zr, f.Name, maxEntryBytes)
			if err == nil {
